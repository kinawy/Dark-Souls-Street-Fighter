...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function playerOneAttack (e) {
    console.log(e.code)
    if (pontiff.x  > artorius.x) {
      pontiff.x  = pontiff.x - 50
    }
  }

  function playerTwoAttack (e) {
    console.log(e.code)
    if (artorius.x < pontiff.x) {
      artorius.x  = artorius.x + 50
    }
  }
  ...
  artorius = new Player2(600, 480, 400, 400);

  // pontiff.render();
  // artorius.render();
  document.addEventListener('keydown', play1Move);
  document.addEventListener('keydown', play2Move);
  document.addEventListener('keydown', playerOneAttack);
  document.addEventListener('keydown', playerTwoAttack);
